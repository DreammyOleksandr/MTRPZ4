...
namespace MTRPZ4.UnitTests.CardServiceTests
{
	public class MakeListOfSortedOptionsTests
	{
			...
			var fonts = new List<Font>
		{
			...
			Assert.Equal(Convert.ToString(19.99M), result[0].ToList()[4].Value);
			Assert.Equal(Convert.ToString(9.99M), result[0].ToList()[5].Value);
			Assert.Equal("Blue", result[1].ToList()[0].Value);
			Assert.Equal("Red", result[1].ToList()[1].Value);
			Assert.Equal(Convert.ToString(19.99M), result[2].ToList()[0].Value);
			Assert.Equal(Convert.ToString(9.99M), result[2].ToList()[1].Value);
